...
@app.get("/api/queue")
async def list_queue(_auth=Depends(require_auth)):
...
class QueueResolve(BaseModel):
    label: str  # 3 options for type, 6 for stage


@app.post("/api/queue/{queue_id}/resolve")
async def resolve_queue_item(
    queue_id: int, req: QueueResolve, _auth=Depends(require_auth)
):
    """Resolve a labeling queue item — user selects the correct label."""
    async with _pool.acquire() as conn:
        item = await conn.fetchrow(
            "SELECT * FROM labeling_queue WHERE id = $1", queue_id
        )
        if not item:
        ...
        # Store as labeled example for future few-shot retrieval
        await conn.execute(
            """
            INSERT INTO labeled_emails (email_id, subject, snippet, stage, confirmed_by)
            VALUES ($1, $2, $3, $4, 'user')
            ON CONFLICT (email_id) DO UPDATE SET stage = $4, confirmed_by = 'user'
            """,
            item["email_id"],
            item["subject"],
            item["snippet"],
...
async def queue_metrics(_auth=Depends(require_auth)):
    """Classification accuracy metrics — auto vs user, correction rate."""
    async with _pool.acquire() as conn:
        auto_count = await conn.fetchval(
            "SELECT COUNT(*) FROM labeled_emails WHERE confirmed_by = 'auto'"
        )
        user_count = await conn.fetchval(
            "SELECT COUNT(*) FROM labeled_emails WHERE confirmed_by = 'user'"
        )
        queue_depth = await conn.fetchval(
            "SELECT COUNT(*) FROM labeling_queue WHERE resolved = FALSE"
        )

    total = (auto_count or 0) + (user_count or 0)
    accuracy = (auto_count or 0) / total if total > 0 else None

    return {
        "total_classified": total,
        "auto_classified": auto_count or 0,
        "user_corrected": user_count or 0,
